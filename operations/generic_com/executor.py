...
def _assert_safe_member(member: str, allowed_members: set[str] | None=None) -> None:
    if member.startswith('_') or member in _BLOCKED_MEMBERS: raise ValueError(f'Blocked unsafe COM member: {member}')
    if allowed_members is not None and allowed_members and member not in allowed_members: raise ValueError(f'COM member is not allowed by whitelist: {member}')
def _resolve_chain(root: Any, chain: tuple[dict[str, Any], ...], allowed_members: set[str] | None=None) -> Any:
    ...
    for item in chain:
        ...
        if not hasattr(current, member): raise ValueError(f'COM member was not found: {member}')
        current=getattr(current, member)
        if 'item' in item: current=current.Item(item['item'])
        args=item.get('arguments')
        if args is not None:
            if not callable(current): raise ValueError(f'COM member is not callable: {member}')
            current=current(*args)
    return current
def execute_generic_com(workbook: Any, operation: OperationSpec, allowed_members: set[str] | None=None) -> Any:
    if operation.parameters.get('advanced_com_enabled') is False: raise ValueError('Advanced COM access is disabled')
    target=_resolve_chain(workbook, operation.target.object_chain, allowed_members); member=str(operation.parameters.get('member',''))
    if not member: raise ValueError('COM operation requires parameters.member')
    _assert_safe_member(member, allowed_members)
    if not hasattr(target, member): raise ValueError(f'COM member was not found: {member}')
    ...
    if operation.opcode=='COM_CALL':
        ...
        if not callable(fn): raise ValueError(f'COM member is not callable: {member}')
        return fn(*(operation.parameters.get('arguments') or []))
    raise ValueError(f'Unsupported COM opcode: {operation.opcode}')
